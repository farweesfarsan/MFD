...
const PaymentSuccess = () => {
  ...
  const sendPDFViaEmail = async () => {
    ...
    try {
      ...
      formData.append('invoice', blob, `Invoice_${order.paymentInfo?.id || 'order'}.pdf`);
      formData.append('email', user.email); 
      formData.append('name',user.name);

      const response = await axios.post('http://localhost:8000/invoice/send-invoice', formData);
        ...
        <div className="flex justify-between gap-10 flex-wrap mb-6 text-gray-700">
          <div className="space-y-2 min-w-[200px] text-left">
            ...
            <div className="flex flex-col items-start gap-2 w-60">
              <p><strong>Address:</strong> {order.deliveryInfo.address}, {order.deliveryInfo.city}</p>
              <p><strong>Contact No:</strong> {order.deliveryInfo.phoneNo}</p>
            </div>
          </div>

          <div className="space-y-2 text-left min-w-[200px]">
            <p><strong>Order ID:</strong> {order.paymentInfo.id}</p>
            <p>
              <strong>Payment Status:</strong>{' '}
              <span className={`
                px-2 py-1 rounded font-medium
                ${order.paymentInfo.status === 'Paid' ? 'bg-green-300 text-green-600' : ''}
                ${order.paymentInfo.status === 'Not Paid' ? 'bg-red-300 text-red-600' : ''}
              `}>
  ...
  {paymentMethod === 'cod' ? 'Cash on Delivery' : 'Online Payment'}
        ...
        {/* Ordered Items */}
        <h3 className="text-2xl text-[#cc3a52] font-semibold oleo-script-bold">Ordered Items</h3>
          ...
          <tbody>
            {order.orderItems.map((item, index) => (
              <tr key={index} className="hover:bg-gray-50">
                <td className="flex items-center gap-3 px-4 py-2 border-b">{item.name}</td>
                <td className="px-4 py-2 border-b">{item.quantity}</td>
                <td className="px-4 py-2 border-b">{item.price}.00</td>
                <td className="px-4 py-2 border-b">{item.price * item.quantity}.00</td>
              </tr>
            ))}
            <tr><td colSpan={4}><hr className="border-t-2 border-gray-700 my-2" /></td></tr>
            <tr className="bg-gray-50 font-medium">
              <td className="px-4 py-2 border-b" colSpan={3}>Delivery Charge</td>
              <td className="px-4 py-2 border-b">{order.deliveryCharge}.00</td>
            </tr>
            <tr className="bg-gray-100 font-bold">
              <td className="px-4 py-2 border-b" colSpan={3}>Total Amount</td>
      ...
      <div className="no-print flex justify-center my-10">
        <button onClick={() => navigate('/')} className="flex items-center gap-3 px-6 py-3 bg-[#1b6b75] text-white text-lg font-medium rounded hover:bg-[#155e67] transition">
